...
public class ProcessaArquivoCDRDadosVoz extends Thread
{
	...
	/**
	 * Metodo....:ProcessaArquivoCDRDadosVoz
	 ...
	 */
	public ProcessaArquivoCDRDadosVoz(long idProcesso) throws GPPInternalErrorException
	{
	...
	public void run()
	{
		// Cria referencia para o parse de arquivos de CDR para Dados/Voz
		ArquivoCDRDadosVoz arqCDRParser = new ArquivoCDRDadosVoz();
		...
		while(deveExecutar)
		{
			// Faz a tentativa de processar um arquivo existente no pool
			File arquivoCDR = null;
			try
			{
				// Busca no pool de arquivos o proximo a ser processado
				// Caso o arquivo exista e tenha sua integridade garantida
				// pelo batimento entao realiza a importacao
				arquivoCDR = gerArquivosCDR.getArquivoDadosVoz();
				if (arquivoCDR != null)
				{
					gerLog.log(idProcesso,Definicoes.INFO,Definicoes.CL_PROCESSA_ARQUIVO_CDR_DADOSVOZ,"run","Processando Arquivo:"+arquivoCDR);
					...
					gerLog.log(idProcesso,Definicoes.INFO,Definicoes.CL_PROCESSA_ARQUIVO_CDR_DADOSVOZ,"run","Arquivo:"+arquivoCDR+" processado.");
				}
				else
				{
					synchronized(this)
			...
			{
				gerLog.log(idProcesso,Definicoes.ERRO,Definicoes.CL_PROCESSA_ARQUIVO_CDR_DADOSVOZ,"run","Erro ao processar arquivo de CDR:"+arquivoCDR + " Erro:"+e);
			}
		}
	}
}
