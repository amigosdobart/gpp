...
{	
			...
			HttpServletResponse response, SessionFactory sessionFactory) throws Exception 
	{
		...
		try 
		{
			...
			request.setAttribute("mensagem", "Conteúdo WIG removido com sucesso!");
		}
		catch (Exception e)
		{
			request.setAttribute("mensagem", "[erro]Erro ao remover Conteúdo WIG . <br><br>" + e);
			logger.error("Erro ao remover Conteúdo WIG . " + e.getMessage());	
