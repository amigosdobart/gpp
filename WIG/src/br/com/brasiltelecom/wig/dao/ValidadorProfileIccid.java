...
/**
 * Esta classe realiza a validacao por iccid retornando entao
 * um objeto resposta alternativo ao valor default para o conteudo
 * 
 * @author Joao Carlos
 * Data..: 16/06/2005
 */
public class ValidadorProfileIccid implements ValidadorConteudo
{
	private static String SQL_PESQUISA =   "SELECT A.CO_RESPOSTA,A.CO_TIPO_RESPOSTA " +
										     "FROM WIGC_VALIDA_PROFILE_ICCID A,HSID_SIMCARD B " +
										    "WHERE A.CO_PROFILE_ICCID = C.NO_PROFILE " +
											  "AND A.NU_ICCID = ? " +
										      "AND A.CO_CONTEUDO = ?";
