...
/**
 * Efetua um ajuste de débito
 * 
 * @author André Gonçalves
 * @since 20/05/2004
 */
public class SalvarAjusteDebitoAction extends ActionPortal{
	...
	public ActionForward performPortal(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response, Database db) throws Exception {
		...
		db.begin();
		logger.info("Ajuste de débito solicitado");
		...
		{
			logger.error("Não foi possível realizar o ajuste de débito, problemas na conexão com o banco de dados (" +
						pe.getMessage() + ")");
			throw pe;
		}	
		...
		ajuste.setTipoAjuste("D");
		
		//Não há mudança na Data de Expiração para Ajuste de Débito.
		ajuste.setDataExpiracao(new Date());
		...
		String tipoCredito = request.getParameter("saldoDebito");
		...
		if (maxValor >= ajuste.getValor()){
			...
			{
				logger.error("Não foi possível realizar o ajuste de débito, problemas com o GPP (" +
							e.getMessage() + ")");
				throw e;
			}
			
			if (ret == 0){
				this.codOperacao = Constantes.COD_AJUSTE_DEBITO;
				request.setAttribute(Constantes.MENSAGEM, "Ajuste de débito de R$ " + 
